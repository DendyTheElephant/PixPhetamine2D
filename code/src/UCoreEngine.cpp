...
using namespace PixPhetamine2D::PostProcess;
using namespace PixPhetamine2D;
using namespace Utility;
using namespace PixPhetamine2D::Display;
...
UCoreEngine::UCoreEngine() : m_isRunning(false) {
	...
	m_InputHandler = new PadPhetamine::CInputHandler(m_SDLWindow);
	m_Camera = new PixPhetamine2D::CCamera(m_SDLWindow);
	...
	// Texture attachment
	m_GBufferMS->addTexture("colorTexture", PixPhetamine2D::LowLevelWrapper::CTexture::ETextureType::NORMAL, 0);
	m_GBufferMS->addTexture("normalTexture", PixPhetamine2D::LowLevelWrapper::CTexture::ETextureType::NORMAL, 1);
	m_GBufferMS->addTexture("typeTexture", PixPhetamine2D::LowLevelWrapper::CTexture::ETextureType::NORMAL, 2);

	m_GBufferAA->addTexture("colorTexture", PixPhetamine2D::LowLevelWrapper::CTexture::ETextureType::NORMAL, 0);
	m_GBufferAA->addTexture("normalTexture", PixPhetamine2D::LowLevelWrapper::CTexture::ETextureType::NORMAL, 1);
	m_GBufferAA->addTexture("typeTexture", PixPhetamine2D::LowLevelWrapper::CTexture::ETextureType::NORMAL, 2);
	m_GBufferAA->addTexture("blurredColorTexture", PixPhetamine2D::LowLevelWrapper::CTexture::ETextureType::NORMAL, 3);

	m_DownSampled->addTexture("processedTexture", PixPhetamine2D::LowLevelWrapper::CTexture::ETextureType::NORMAL, 0);
	m_RGBSplitted->addTexture("processedTexture", PixPhetamine2D::LowLevelWrapper::CTexture::ETextureType::NORMAL, 0);
	m_BufferBlurPartial->addTexture("processedTexture", PixPhetamine2D::LowLevelWrapper::CTexture::ETextureType::NORMAL, 0);
	m_BufferBlur->addTexture("processedTexture", PixPhetamine2D::LowLevelWrapper::CTexture::ETextureType::NORMAL, 0);
	...
	m_skyBox = new PixPhetamine2D::SceneRendering::CSkybox("textures/skybox");	
...
UCoreEngine::~UCoreEngine() {
	STACK_TRACE;
	PixPhetamine2D::Display::shutdownSDL_GL(m_SDLWindow, m_GLContext);
...
void UCoreEngine::loadShaders() {
	...
	for (auto const &it_shaderName : m_ShaderNames) {
		...
		m_ShaderList[it_shaderName] = new PixPhetamine2D::LowLevelWrapper::CShader(vertexShader.c_str(), fragmentShader.c_str());
	}
	UNSTACK_TRACE;
}
...
void UCoreEngine::loadMeshes() {
	...
	for (auto const &it_meshName : m_MeshNames) {
		...
		m_MeshList[it_meshName] = new PixPhetamine2D::CStaticMesh(meshPath.c_str());
	}
	UNSTACK_TRACE;
}
...
void UCoreEngine::runGameLoop() {
	...
	do {
		...
		if (m_InputHandler->getKey(PadPhetamine::CInputHandler::EInput::LEFT)) {
			m_Camera->moveCameraLeft(speed);
		}
		if (m_InputHandler->getKey(PadPhetamine::CInputHandler::EInput::RIGHT)) {
			m_Camera->moveCameraRight(speed);
		}
		if (m_InputHandler->getKey(PadPhetamine::CInputHandler::EInput::UP)) {
			m_Camera->moveCameraForward(speed);
		}
		if (m_InputHandler->getKey(PadPhetamine::CInputHandler::EInput::DOWN)) {
			m_Camera->moveCameraBackward(speed);
		}
		if (m_InputHandler->getKey(PadPhetamine::CInputHandler::EInput::SHOULDER_RIGHT)) {
			m_Camera->moveCameraUp(speed);
		}
		if (m_InputHandler->getKey(PadPhetamine::CInputHandler::EInput::SHOULDER_LEFT)) {
		...
		PixPhetamine2D::LowLevelWrapper::initialiseDrawIntoBuffer(m_ShaderList["basic"]->id(), m_GBufferMS->getId(), gBufferTargets, 3);
		...
		glBindVertexArray(m_MeshList["cube"]->getVBO());
		...
		for (size_t i_lionhead = 0; i_lionhead < 10; ++i_lionhead) {
			...
			glDrawElements(GL_TRIANGLES, m_MeshList["cube"]->getNumberOfFaces(), GL_UNSIGNED_SHORT, (void *)0);
		}
		glBindVertexArray(0);
		...
		/* =========================================================================================== */
		//PixPhetamine2D::LowLevelWrapper::multiSamplingAntiAliasing(m_GBufferMultiSampled, m_GBufferWitoutAliasing, WINDOW_WIDTH, WINDOW_HEIGHT);
		STACK_TRACE;
