...
using namespace CommonPhetamine::Geometry;
...
namespace PixPhetamine2D {
	...
	void CCamera::moveView(pxFloat const& a_dx, pxFloat const& a_dy) {
		...
		m_direction.x = cosf(radAngleY) * sinf(radAngleX);
		m_direction.y = sinf(radAngleY);
		m_direction.z = cosf(radAngleY) * cosf(radAngleX);
