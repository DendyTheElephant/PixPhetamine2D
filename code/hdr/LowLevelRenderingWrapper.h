...
namespace PixPhetamine2D {
	...
	namespace LowLevelWrapper {
		...
		struct VAO {
			...
			void loadToGPU(pxVec<pxFloat> &vertices, pxVec<pxFloat> &normals, pxVec<pxFloat> &colors, pxVec<pxUInt16> &faces, GLenum mode);
			void free();
		};

		/// \brief		GBuffer is a custom FBO (image buffers) to PixPhetamine2D the scene and apply some postprocess job
		...
		struct GBuffer {
			...
			GLvramLocation	VAO_id{ 0 };			///< ID of the VAO, usefull to PixPhetamine2D the quad texture
		...
		/// \brief		ImageBuffer is a custom FBO to PixPhetamine2D in, usefull for many effects.
		/// \details	Use this to PixPhetamine2D in a custom texture, apply processes, PixPhetamine2D in other buffers then display on screen
		///				Don't forget to free() after using!
		struct ImageBuffer {
